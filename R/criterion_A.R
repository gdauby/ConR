...
criterion_A = function(x, 
  ...
  if (max(project.years) - max(years) > 100) {
    stop("Projected population sizes are larger than 100 years from last year of observation")
  }
  ...
  if(is.null(generation.time)) {
    ...
    warning(
      "generation time not given: assuming 10 years previous to the year of assessment. Please, check if this is accurate for your species"
    )
    ...
    if (!min(all.yrs) %in% years)
      yrs = c(min(all.yrs), yrs)
    if (!max(all.yrs) %in% years)
      yrs = c(yrs, max(all.yrs))
    
  }
  
  ## if the first column indicate taxa name
  ...
  # TO CHECK FOR VARIOUS COMBINATION IF CORRECTs
  all_ranks <- cat_criterion_a(
    A1_val = Results$reduction_A12,
    A2_val = Results$reduction_A12,
    A3_val = Results$reduction_A3,
    A4_val = Results$reduction_A4, 
    A1.threshold = A1.threshold, 
    A234.threshold = A234.threshold
  )
  
  Results$category_cA <- all_ranks$ranks_A1234
  Results$Category_code <- all_ranks$cat_codes
