...
#' @param threat.map raster, raster layer/stack or spatial polygons (simple feature) indicating land cover unsuitable to taxa
...
AOO.decline <- function(XY,
  ...
  if (!any(class(threat.map) == "list")) 
    threat.map <- list(threat.map)
  
  ids_to_keep <- vector('list', length(threat.map) + length(hab.class))
  ...
  if (all_individual_layers & length(length(ids_to_keep)) > 1) {
    ...
    for (i in 1:length(AOO_res_list)) {
      AOO_treath <- data.frame(AOO_minus_treats = AOO_res_list[[i]], 
